<template>
  <PageLayout
    :title="`${t('downloads.title')} (${downloads.length})`"
    icon="mdi:download"
  >
    <div
    ...
    <template v-else>
      <Section
        v-for="(group, date) in groupedDownloads"
        :key="date"
        :title="dayLabel(date)"
      >
        <SectionItem
        ...
        >
              ...
              <div class="download-text">
                <div class="download-name">
                  ...
                  <span
                    ...
                    class="download-deleted-badge"
                  >{{ t('downloads.deleted') }}</span>
                </div>
          ...
          <IconButton
            ...
            :size="18"
            :tooltip="t('files.openInBrowser')"
            @click.stop="handleOpenInBrowser(item.path)"
          />
          <IconButton
...
async function handleOpenInBrowser(path: string) {
  console.debug('[DownloadsView] handleOpenInBrowser: path', path)
  await window.browserAPI.openFileInBrowser(path)
}
